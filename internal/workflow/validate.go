	...
	if cfg.Agent.MaxTokensPerClaim < 0 {
		return fmt.Errorf(
			"%s: agent.max_tokens_per_claim must be non-negative "+
				"(counts only worker-observed, runner-reported Codex usage; external GitHub @codex review, other reviewers outside the worker session, and otherwise unreported nested or subagent usage are excluded)",
			path,
		)
	}
