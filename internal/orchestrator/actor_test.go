...
func TestRecordRuntimeEventBudgetExceededCancelsAndBlocksRun(t *testing.T) {
	...
	blocked := view.Blocked[0]
	wantBudgetError := "worker-observed, runner-reported Codex claim token budget exceeded: current_claim_total_tokens=12 max_tokens_per_claim=10; external review and otherwise unreported nested or subagent usage are excluded"
