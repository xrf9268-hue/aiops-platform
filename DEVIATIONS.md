...
| D33 | Worker-side post-run gates (`verify`, secret-scan, RUN_SUMMARY, policy) run as orchestrator phases on the wrong side of the SPEC §1 agent boundary (2026-05-15 audit "Deliberate extensions" items 4/5/7 + RUN_SUMMARY). The worker `verify` gate is removed here — verification is the agent's responsibility, surfaced to the prompt via `AppendVerifyDirective` and backstopped by PR CI; the race in #557 where reconcile-cancel skipped the post-turn worker verify is structurally eliminated. secret-scan / RUN_SUMMARY / policy remain and are tracked as follow-ups. | §1 boundary | Medium | Partial — verify gate removed; secret-scan/RUN_SUMMARY/policy follow-up | #557 / umbrella [#67](https://github.com/xrf9268-hue/aiops-platform/issues/67) |
