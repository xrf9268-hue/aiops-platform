...
| D29 | `agent.max_retry_attempts` / `agent.max_timeout_retries` caps and the `OrchestratorState.Failed` map (+ `ReleaseFailedIfIssueChanged`) are non-SPEC harness extensions. **The opt-in retry caps + the `maxFailureRetries` machinery were removed in #577**, restoring the SPEC §8.4 / §16.6 unbounded-retry-with-backoff default as the only path; the keys are rejected at load. The `Failed` map is **retained** — after the cap removal its only remaining writer is `applyNonRetryable` (deterministic non-retryable failures such as a malformed `WORKFLOW.md`, which would otherwise spin on every poll tick, regression #234). Whether that deterministic-suppression sub-behavior is itself SPEC-aligned or further over-design is tracked separately (the #577/#576 "second-to-land deletes the orphaned Failed map" framing was based on an incomplete writer census — the map is not orphaned). | §4.1.8, §8.4, §15.5, §16.6 | Low | Partial — opt-in caps removed (#577); `Failed` deterministic-suppression retained, evaluation tracked by [#584](https://github.com/xrf9268-hue/aiops-platform/issues/584) | [#215](https://github.com/xrf9268-hue/aiops-platform/issues/215) / [#577](https://github.com/xrf9268-hue/aiops-platform/issues/577) / [#584](https://github.com/xrf9268-hue/aiops-platform/issues/584) |
