...
- Added loader regression tests in `internal/workflow/loader_test.go` for both the single-service missing-slug case and the service-route missing-slug case.
...
- `go test -race -covermode=atomic ./...` was run and still fails only in `internal/runner` on this macOS host: `TestCodexAppServerRunnerServerRequestsRefreshStallClock` exceeded its 100ms stall budget, and Linux sandbox backend tests return `requires linux host OS; current host OS is darwin`.
