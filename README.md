...
| `agent.max_continuation_turns` | `agent.max_turns` (default `20`) cumulative clean-turn budget across continuation re-dispatches; reaching it parks the issue in local `blocked` state (`continuation_budget`) instead of looping forever | implementation (accepted deviation D34 / #621) |
