...
- **Dispatcher wait pattern**: use `--kind done` / `--kind turn_finished` (trellis-emitted system events), NOT invented user tags as the completion signal. The v0.6.x channel CLI has no `--tag` flag for `send`, `wait`, or `messages`; relying on a worker to emit a custom tag-shaped string is unreliable and cannot be filtered by the CLI. See `references/command-reference.md` "tag vs kind".
