...
| D4 | Closed after #72 | Multi-path discovery (`WORKFLOW.md`, `.aiops/`, `.github/`) was removed after this audit. The resolver now considers only the canonical `WORKFLOW.md`; legacy alternate paths are ignored and not reported as shadow sources. |
...
| `cmd/worker/main.go:17-23` (`--print-config`) | Debug subcommand that prints the resolved workflow + masked secrets. | Survives — pure observability, SPEC §13 is permissive. After #72, this no longer normalizes legacy multi-path workflow discovery. |
