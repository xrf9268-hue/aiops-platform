...
continues, with direct-process kill as a group-signal fallback. Shutdown is
complete only after every process group is confirmed absent; otherwise the
supervisor records `workers_stop_incomplete`, reports the live PIDs, and never
marks the run stopped. Every worker log close is attempted; close failures are
accumulated, cannot mask a termination-evidence error, and cannot trigger a
second shutdown after workers are already stopped.
