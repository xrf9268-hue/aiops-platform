...
repo:
  owner: zjlgdx
  name: aiops-workflow-bench-high-20260717-1128-v1
  clone_url: https://github.com/zjlgdx/aiops-workflow-bench-high-20260717-1128-v1.git
  default_branch: main
...
codex:
  command: codex app-server --config shell_environment_policy.inherit=all --config model=gpt-5.6-sol --config model_reasoning_effort=high
...
This is the fixed high-assurance profile: the full unseen-tuple review must
also run targeted adversarial negative and failure-path probes against the real
built process. Do not create a nested/subagent/local-review layer; you are the
only local reviewer.
...
2. **Verification (you own this):** only for an unseen tuple, run these commands
   once: `gofmt -l .`; `go test ./...`; `go vet ./...`.
   The same-tuple checkpoint path skips them.
3. Review the complete diff against the issue, including behavior-level tests,
   security, failure paths, and unrelated scope. Derive at least one malformed
   or boundary-input probe and one operational failure-path probe from the
   changed behavior, execute them against the real built CLI/process, and
   require exact exit/output/persistence behavior. A test-only or mocked probe
   is insufficient for this high profile.
...
This high profile requires exactly one GitHub Codex trigger for each locally
passing unseen exact tuple. Reuse an existing exact-tuple trigger; post at most one
`@codex review` trigger per tuple. After creating the exact local checkpoint,
wait at least two seconds, run the tuple-only guard, then post the trigger as
the reviewer identity. This delay makes the checkpoint's whole-second GitHub
timestamp strictly earlier than the trigger; if the tuple changed, post
nothing and end. Accept only a REST review object from bot numeric id
`199175422`, with `commit_id=<HEAD>` and `submitted_at` at or after that exact
trigger. Comments, reactions, clean-looking prose, or absence of findings are
advisory only. The absence of this reliable signal is not clean. Findings join
the FAIL evidence: submit one consolidated commit-pinned `CHANGES_REQUESTED`,
then use the `aiops:rework` transition above as the LAST action. No-signal,
