...
event-driven loop (L3). Recurring reviewer findings can inform ordinary,
reviewed changes to `WORKFLOW.md`, rubrics, `LEARNINGS.md`, skills, hooks, or
CI, but this runbook does not add an automatic outer loop or worker-side
post-turn verifier. Zach Lloyd's self-improving Skills account uses the same
review boundary: past runs plus human or grader feedback produce a reviewed
harness diff.
...
- The pattern satisfies LangChain L2 without adding an automatic L4 loop:
  recurring reviewer findings may inform ordinary harness PRs, which remain
  subject to the repository's normal review process.
