...
| Linux amd64 Docker worker | Build `Dockerfile` target `codex-worker`; Codex CLI `0.142.0` is installed from a pinned release artifact and checksum. | Use base `worker` target for mock-only validation. | Online installer is not the promoted Docker path until Linux ARM64 checksum behavior is reliable. |
