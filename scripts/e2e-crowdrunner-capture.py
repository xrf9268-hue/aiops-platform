#!/usr/bin/env python3
"""Capture evidence for the local Crowd Runner product lifecycle E2E."""
...
def parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--maker-url", default=os.getenv("AIOPS_CROWDRUNNER_MAKER_DASHBOARD_URL", ""))
    p.add_argument("--reviewer-url", default=os.getenv("AIOPS_CROWDRUNNER_REVIEWER_DASHBOARD_URL", ""))
    p.add_argument("--stress-url", default=os.getenv("AIOPS_CROWDRUNNER_STRESS_DASHBOARD_URL", ""))
    p.add_argument("--gitea-url", default=os.getenv("AIOPS_CROWDRUNNER_GITEA_URL", ""))
    p.add_argument("--product-url", default=os.getenv("AIOPS_CROWDRUNNER_PRODUCT_URL", ""))
    ...
    p.add_argument("--tui-bin", default=os.getenv("AIOPS_CROWDRUNNER_TUI_BIN", "tui"))
...
def default_screenshots(args: argparse.Namespace) -> list[tuple[str, str]]:
    ...
    if args.stress_url:
        shots.append(("stress-dashboard", args.stress_url))
    if args.product_url:
        shots.extend([
            ("product-gameplay", args.product_url),
            ("product-mobile", args.product_url),
        ])
    return shots
    ...
    with sync_playwright() as pw:
        ...
        for name, url in shots:
            ...
            try:
                ...
                if required:
                    raise
                print(f"skipped default screenshot {name} ({url}): {exc}")
                continue
...
def main() -> int:
    ...
    fetch_json(args.stress_url, dirs["state"] / f"stress-{args.tag}.json", args.state_token)
    ...
    capture_tui(
        args.tui_bin,
        args.stress_url,
        dirs["pages"] / f"tui-stress-{args.tag}.txt",
        args.tui_seconds,
        args.state_token,
    )
    ...
    if not args.no_default_screenshots:
        entries.extend(
            capture_screenshots(default_screenshots(args), dirs["screenshots"], args.tag, False)
        )
    if explicit_shots:
        entries.extend(capture_screenshots(explicit_shots, dirs["screenshots"], args.tag, True))
    if entries:
